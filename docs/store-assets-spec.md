...
## Rendered assets (Phase 28)

Generated by `tool/generate_store_assets.dart` (pure Dart, `image` package — reproducible):
`dart run tool/generate_store_assets.dart`.

- `assets/app_icon/app_icon.png` (1024² master) — **wired** into the Android mipmaps + adaptive
  icon via `dart run flutter_launcher_icons`; the default Flutter logo is gone.
- `assets/app_icon/app_icon_foreground.png` (adaptive fg, central safe zone) ·
  `…/app_icon_background.png` (solid `#0D0E12`).
- `assets/store/play-icon-512.png` (Play listing icon).
- `assets/store/feature-graphic-1024x500.png` — **text-free v1** (mark + composition only).

**Feature graphic — text-overlay spec (design-tool pass):** over the text-free v1, add the
wordmark **CHESS RESCUE** (Inter Tight, `#EAEAF2`, tight tracking) + tagline *"One move saves the
king."* (textDim) in the **right ~55% negative space**, vertically centered, left-aligned, within
the ~924×400 safe area. Deliberately kept out of the generator (bitmap type reads cheap on a hero
asset).

The marks are an **algorithmic baseline** — a designer can refine the vector from here.
